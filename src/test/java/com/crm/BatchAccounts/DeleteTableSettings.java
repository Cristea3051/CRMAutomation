...
public class DeleteTableSettings {
  ...
  public void setUp() {
    ...
    options.addArguments("--headless"); // Rulează în mod headless
    options.addArguments("--disable-gpu"); // Opțional, pentru compatibilitate
    options.addArguments("--window-size=1920,1080"); // Setează o rezoluție standard
    options.addArguments("--no-sandbox"); // Util pentru rularea pe servere CI/CD
    options.addArguments("--disable-dev-shm-usage"); // Evită erori de memorie pe Linux

    driver = new ChromeDriver(options);  // Folosește opțiunile configurate
    ...
    public void signIn(String username, String password) {
        ...
        WebElement select = wait
                .until(ExpectedConditions.visibilityOfElementLocated(By.name("accounts-batch-list_length")));
